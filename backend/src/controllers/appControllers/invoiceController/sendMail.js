...
const mail = async (req, res) => {
  ...
  const fileUrl = `${process.env.PUBLIC_SERVER_FILE}/download/invoice/invoice-${jsondata?._id}.pdf`;
  const fileName = `invoice-${jsondata?._id}.pdf`;
  ...
  try {
    ...
    const mailOptions = {
      ...
      to: client?.email,
      subject: 'Your Invoice is ready now',
      html: htmlContent,
      attachments: [
        {
          filename: fileName,
