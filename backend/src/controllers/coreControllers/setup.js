...
const setup = async (req, res) => {
  ...
  for (const filePath of settingsFiles) {
    ...
    const settingsToUpdate = {
      custflow_app_email: email,
      custflow_app_company_email: email,
      custflow_app_timezone: timezone,
      custflow_app_country: country,
      custflow_app_language: language || 'en_us',
    };
  ...
  return res.status(200).json({
    success: true,
    result: {},
    message: 'Successfully CustFlow App Setup',
  });
};
